...
public interface ConversationRepository extends JpaRepository<Conversation, String> {
    ...
    SELECT
      ...
      -- DIRECT: lấy name/avatar của đối phương; ngược lại: dùng của conversation
      ...
      -- isBlocked: chỉ tính cho direct, khi mình chặn họ hoặc họ chặn mình
      CASE
        WHEN c.type = 'direct'
             AND (bu1.id IS NOT NULL OR bu2.id IS NOT NULL)
          THEN 'true'
        ELSE 'false'
      END AS isBlocked
    ...
    -- block hai chiều (chỉ khi direct)
    LEFT JOIN blocked_user bu1
      ON c.type = 'direct'
     AND bu1.user_id = :userId
     AND bu1.blocked_user_id = u2.user_id
    LEFT JOIN blocked_user bu2
      ON c.type = 'direct'
     AND bu2.user_id = u2.user_id
     AND bu2.blocked_user_id = :userId
